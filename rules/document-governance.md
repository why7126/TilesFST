...
### 2.5 OpenSpec specs 语言与占位文案治理（MUST）

`openspec/specs/**/spec.md` 是已生效能力规范，正文面向项目团队长期维护，MUST 遵守 `rules/language.md`：

- `Purpose` MUST 使用中文说明能力边界、适用对象和维护范围。
- 归档后 MUST 清理 `TBD - created by archiving...` 等脚手架占位文案。
- 业务需求标题、场景名称、行为描述和验收条件 MUST 使用中文。
- OpenSpec CLI 解析关键字（`Requirement`、`Scenario`、`MUST`、`SHALL`、`WHEN`、`THEN`、`AND`）MAY 保留英文；不得为了中文化破坏 OpenSpec 校验格式。
- API 路径、HTTP 方法、字段名、枚举值、表名、文件路径、命令与产品英文专名 MAY 保留英文。
- 新增或归档能力时，归档前 SHOULD 检查生成后的正式 spec 是否存在英文脚手架占位文案；发现后应在对应 OpenSpec Change 中同步修正。
