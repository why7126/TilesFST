...
- `deploy/**/*.env.example` 可提交；真实 `deploy/**/*.env`、`deploy/**/*.env.local`、`deploy/**/*.env.prod` 可作为本地/生产运行配置存在于工作区，但禁止 Git 跟踪、暂存或提交。
...
- 生产维护类一次性作业必须通过独立 Compose profile 或等价受控入口启用；若复用既有 `OBJECT_STORAGE_*`、`DATABASE_URL`、镜像仓库和 tag 变量，不得新增重复 env。维护作业 env 示例只允许占位值与注释，不得提交真实生产连接串、密钥、bucket 截图或本机路径。
...
- 生产维护作业 Compose profile、服务命令、env 透传或备份门禁
