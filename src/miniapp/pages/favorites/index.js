...
Page({
  ...
  onSearchInput(event) {
    this.setData({ keyword: (event.detail && event.detail.keyword) || '' });
  },

  onSearchSubmit(event) {
    const keyword = String((event.detail && event.detail.keyword) || '').trim().replace(/\s+/g, ' ');
  ...
  favoriteEmptyText(filteredCount, allCount, keyword = this.data.keyword) {
