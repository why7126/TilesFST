...
Page({
  ...
  scheduleSearchInputTrack(keyword, normalizedKeyword, requestId) {
    if (this.inputTrackTimer) {
      clearTimeout(this.inputTrackTimer);
    }
  ...
  loadResults(reset) {
    ...
    request(`/api/v1/miniapp/search?${params}`)
      .then((data) => {
        ...
        const resultCount = reset ? this.searchResultCount(data) : Math.max(this.searchResultCount(data), this.data.total, items.length);
        ...
        const bestMatch = reset ? this.normalizeSearchItem(data.best_match || null, '最佳匹配') : this.data.bestMatch;
        this.setData({
          items,
  ...
  mergeDisplaySections(currentSections, incomingSections) {
    const identityOf = (item, entityType) => String(item.product_id || item.id || item.sku_code || `${entityType}:${item.card_title || item.title || item.name || ''}`);
    ...
    incomingSections.forEach((incoming) => {
      ...
      byType.set(incoming.entity_type, Object.assign({}, current, {
        count: Math.max(current.count || 0, incoming.count || 0),
        items: current.items.concat(appended),
      }));
    });
    return ['brand', 'certificate', 'sku']
      .map((entityType) => byType.get(entityType))
      .filter((section) => Boolean(section));
  },

  normalizeSearchItem(item, fallbackLabel) {
    if (!item) return null;
