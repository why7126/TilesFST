...
Page({
  data: {
    ...
    activeFilterChips: [] as Array<{ key: string; label: string }>,
      ...
      .then((data) => {
        ...
        this.setData({
          ...
          facets: {
            ...
            price_ranges: data.facets?.price_ranges || [],
          },
  ...
  removeFilter(event: WechatMiniprogram.TouchEvent) {
    const key = String(event.currentTarget.dataset.key || '');
    this.setData({
  ...
  changeSort(event: WechatMiniprogram.TouchEvent) {
  ...
  buildFilterChips(): Array<{ key: string; label: string }> {
    const filters = this.data.filterSnapshot;
  ...
  priceLabel(value: string): string {
