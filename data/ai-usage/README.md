...
updated_at: 2026-07-22 19:40:00
---
...
- Sprint 使用量矩阵：`usage_matrices` 仅保存 workflow event 标签、对象 ID 和数字指标，用于 `/sprint-exps` 输出 `total_tokens`、`input_tokens`、`output_tokens`、`model_call_count` 四张矩阵。
...
- `usage_matrices` 存在，并包含 `total_tokens`、`input_tokens`、`output_tokens`、`model_call_count` 四个指标的矩阵数据
...
Sprint snapshot 的 `usage_matrices` 用于 `/sprint-exps` 的模型 Token 使用分析，结构包含：

- `columns`：固定命令列，按 `Capture`、`BUG-Capture`、`REQ-Capture`、`BUG-Explore`、`REQ-Explore`、`REQ-Generate`、`BUG-Generate`、`REQ-Complete`、`BUG-Complete`、`REQ-Review`、`BUG-Review`、`REQ-Opsx`、`BUG-Opsx`、`Opsx-Explore`、`Opsx-Propose`、`Opsx-Apply`、`Opsx-Archive`、`Sprint-Propose`、`Sprint-Explore`、`Sprint-Apply`、`Sprint-Archive` 排序。
- `rows`：第一行 `Total`，第二行 Sprint，之后按 canonical REQ ID、canonical BUG ID 排序。
- `metrics`：每行包含 `total_tokens`、`input_tokens`、`output_tokens`、`model_call_count`。

口径：`Total` 与 Sprint 行按唯一 command run 汇总；REQ/BUG 行是对象归因视图。同一 command run 同时关联多个 REQ/BUG 时，可计入多个对象行，因此对象行不应直接相加后与 `Total` 比较。
