...
title: 缺陷追踪
purpose: BUG-0045 系统设置媒体格式选项过少
content: 记录媒体与存储 Tab 图片/视频格式各仅 3 种
owner: product
status: done
...
bug_name: system-settings-media-format-options-limited
severity: medium
status: done
iteration: sprint-003
related_requirement: REQ-0017-system-settings
related_bug: null
related_change: fix-system-settings-media-format-options
suggested_fix_change: fix-system-settings-media-format-options
target_clients:
  web_admin: 是
...
openspec_changes:
  - change_id: fix-system-settings-media-format-options
    type: fix
    status: archived```
...
| 时间 | 事件 | 说明 |
|---|---|---|
| 2026-06-28 17:53:48 | `/capture` | 记录媒体与存储格式选项各仅 3 种 |
...
| 2026-06-28 18:45:00 | `/bug-opsx` | 创建 change `fix-system-settings-media-format-options` |
