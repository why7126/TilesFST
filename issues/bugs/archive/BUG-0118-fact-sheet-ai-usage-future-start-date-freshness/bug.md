...
title: Fact Sheet AI usage fresh gate 将未来 Sprint start_date 当作 snapshot 新鲜度下限
severity: medium
status: done
owner:
discovered_at: 2026-08-06 08:35:36
environment: local
related_requirement:
related_change:
related_bug: BUG-0113-fact-sheet-ai-usage-fresh-gate-snapshot
created_at: 2026-08-06 08:39:23
updated_at: 2026-08-06 12:03:56
---
...
- 影响 `sprint-020` 等存在未来计划日期但已归档或已生成当前 AI usage snapshot 的 Sprint。
- 影响 `/sprint-exps`、Fact Sheet summary 与 `--ai-usage-markdown` 的真实 token 成本矩阵输出。
- 影响 AI usage 成本复盘的可信度，可能让完整 snapshot 被误降级为估算兜底。

## 严重等级说明

严重等级为 `medium`。该问题不阻塞核心业务运行，也不直接影响线上用户数据；但会导致 Sprint 复盘和 Fact Sheet 的 AI usage 成本分析失真，影响治理报表与后续成本判断。
