...
updated_at: 2026-07-01 13:59:48
...
已确认使用 **Web 层代理 Swagger**。

后续修复 Change MUST 在 Web 层补齐 Swagger 代理策略，保持页面 `href="/docs"` 可用，并避免在前端硬编码后端宿主机端口。

实施时至少覆盖：

- Vite dev proxy：补充 Swagger UI 相关路径代理，使本地 Web 开发环境访问 `/docs` 时进入 FastAPI Swagger UI。
- Docker Nginx：补充 `/docs`、`/redoc` 以及 Swagger UI 依赖的 OpenAPI/静态资源相关代理，使 Docker Web 端口访问 `/docs` 时进入后端 Swagger UI。
- 现有代理不回归：`/api/`、`/media/`、`/openapi.json` 仍按当前策略转发。
- 生产策略不放松：生产环境仍禁用或隐藏 Swagger `Try It Out`，不因 Web 代理而放开调试能力。
