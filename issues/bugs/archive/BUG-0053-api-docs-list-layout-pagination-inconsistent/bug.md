...
产品确认：用户反馈中的「第一行【系统接口】信息」指接口列表区标题，修复时应直接移除该标题，不改名、不作为真实接口数据行过滤处理。
...
- 接口文档列表直接移除冗余的「系统接口」标题，列表信息层级与管理端其他列表页一致。
- 接口列表提供分页能力与分页 UI。
...
- 接口列表区域存在「系统接口」标题，已确认为需要直接移除的冗余标题。
- 接口列表没有真实分页控件。
...
- 参考文件：
  ...
  - `issues/requirements/archive/REQ-0022-admin-api-docs-menu/acceptance.md`
