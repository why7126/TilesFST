...
- [ ] AC-001 每个 `/req-*`、`/bug-*`、`/opsx-*`、`/sprint-*` source-command 技能 MUST 在主命令成功且 Workflow Sync 成功后触发统一 AI usage fact source hook，或明确引用同一后置流程。
...
- [ ] AC-011 source-command skill MUST NOT 复制大段 `extract-ai-usage` 解析逻辑；只允许声明调用时机、关键参数和输出要求。
...
- [ ] AC-060 实现 MUST 更新相关 source-command skill 或共享工作流规则，使 `/req-*`、`/bug-*`、`/opsx-*`、`/sprint-*` 的后置 Token 事实源构建行为有统一说明。
...
- [ ] AC-072 MUST 验证 source-command skill 或共享规则中没有复制原始 session、prompt、工具输出正文的要求。
