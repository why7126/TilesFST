...
| 流程维护者 | 用统一脚本和规则维护事实源构建逻辑，避免所有 source-command skill 重复粘贴实现细节。 |
...
| 范围 | 说明 |
|---|---|
| 工作流命令族 | 覆盖 `/req-*`、`/bug-*`、`/opsx-*`、`/sprint-*` source-command 技能。 |
...
| 技能与规则同步 | 更新相关 source-command skill、`data/ai-usage/README.md`、必要时更新 `rules/agent-context-budget.md` 或工作流规则。 |
...
系统 MUST 通过统一脚本、函数或等价封装执行自动构建逻辑，避免在每个 source-command skill 中复制 `extract-ai-usage` 的长命令与判断分支。
...
source-command skill SHOULD 只声明统一 usage hook 的调用时机和输出要求，不应粘贴复杂解析逻辑。

候选改造方式：

- 在通用规则中定义 `Final Step — AI Usage Fact Source`；
- 或在每个 source-command skill 的 Final Step 中引用同一个脚本命令；
...
| 相关技能 | `.agents/skills/source-command-*` | 本需求涉及其 Final Step 或统一后置步骤。 |
