...
owner: product
source: issues/requirements/plan/REQ-0013-admin-shell-padding-refine/capture.md
priority: P1
...
`/req-explore` 已确认：本需求为 **REQ-0004** 的布局 refine；**不改动**侧栏列宽 264↔72（`REQ-0011` 契约）；**不改动** `main-content` bottom padding（保持 72px）；content-inner 采用 **策略 B** 软上限 `min(1400px, 100%)`。
...
- 主内容 `.main-content` 上/左右 padding 收窄；**bottom 保持 72px**
- `.content-inner` 统一 fluid：`max-width: min(1400px, 100%)`；`margin: 0 auto` 保留
...
- Prototype（`/req-complete` 补齐）：expanded 1440/1920、collapsed、≤1023 一帧 + context.md
- Vitest 回归：`AdminSidebar` collapse、`AdminLayout` smoke；collapsed 态无 `.nav-scroll` 横向溢出
...
- `.main-content` padding MUST 自 `48px 56px 72px` 调整为 **`32px 32px 72px`**。
- **bottom padding MUST 保持 72px**，本 REQ 不得修改。
- 主内容区 MUST 仍 `overflow: auto`；不得因 padding 变更引入横向滚动条（内容本身超宽除外）。

### FR-002 侧栏 padding — expanded（desktop >1023px）

- `.sidebar` padding MUST 自 `30px 18px 18px` 调整为 **`30px 6px 18px`**（仅收窄水平方向）。
...
- `[data-sidebar-state='collapsed'] .sidebar` padding MUST 自 `16px 8px 18px` 收紧为 **`12px 6px 14px`**（prototype 可在 ±2px 内微调，须更新 prototype Golden）。
- 水平 padding MUST 与 expanded 6px 壳层语义一致。
...
### FR-004 Content-inner fluid — 策略 B
...
- 实现 SHOULD CSS 变量化，例如 `--admin-content-max-width: min(1400px, 100%)`。
...
- 在 1920×1080 expanded 侧栏下，content-inner 实际宽度 MUST 为 **1400px**（受软 cap 限制）；在 1440×1024 下 MUST 随可用宽度 fluid 至约 **1112px** 量级（264 侧栏 + 32×2 main padding 后）。

### FR-005 响应式 padding 联动

视口 **≤1023px**（tablet / 窄桌面）：

- MUST 联动调整 `.sidebar` 与 `.main-content` padding；**MUST NOT** 对全宽侧栏 grid 沿用 desktop 6px 水平 padding。
- 建议目标（prototype 定稿 Golden）：sidebar **`18px 16px`** 量级；main-content 在现 `32px 24px 56px` 基础上略收（如 **`24px 20px 56px`**）。
- MUST 保持 `REQ-0011` 既有结构：侧栏置顶、双列 nav、隐藏 sidebar-user、chevron 隐藏。

视口 **≤639px**（mobile）：

- MUST 联动调整 main-content padding，在现 `28px 18px 48px` 基础上略收（prototype 定稿，如 **`24px 16px 40px`**）。
...
- 验收时 content 区左右视觉 gutter（侧栏右缘至 content-inner 左缘、content-inner 右缘至视口右缘）SHOULD 显著小于变更前 1080px 居中方案；具体阈值于 `/req-complete` 写入 `acceptance.md`（引用 prototype 标注）。
...
**建议 OpenSpec change 命名**：`fix-admin-shell-padding-refine`。
...
owner: product
parent_requirement: REQ-0004-admin-home
openspec_change: null  # 建议 fix-admin-shell-padding-refine
target_clients:
  web_admin: 本期
  web_catalog: 不涉及
  wechat_miniapp: 不涉及
