...
updated_at: 2026-08-26 10:34:15
...
| 类型 | 编号 | 标题 | 状态 | 说明 |
|---|---|---|---|---|
| BUG | BUG-0141-ai-usage-token-count-jsonl | AI usage extractor 未识别新版 token_count JSONL 导致 Sprint snapshot 缺失 | in_sprint | Change 已 apply，待 archive |
| BUG | BUG-0140-admin-current-user-avatar-missing-object | 当前登录用户头像引用缺失媒体对象 | in_sprint | Change 已 apply，待 archive |
| BUG | BUG-0139-admin-avatar-upload-nginx-redirect-cors | 管理后台头像上传被 Nginx 301 重定向丢端口导致 CORS 拦截 | in_sprint | 待创建修复 Change |
| REQ | REQ-0123-upload-stage-trace-spans | 上传链路阶段级耗时写入 trace spans | in_sprint | 待创建 OpenSpec Change |
| REQ | REQ-0124-log-audit-behavior-trace-model | 日志审计补齐行为链路与任务链路采集模型 | applied | Change 已 apply，待 archive |
| REQ | REQ-0126-product-data-collection-observability-standard | 建立通用产品数据采集与链路观测规范 | in_sprint | 待创建 OpenSpec Change |
