---
note: workflow-sync — workflow-sync 自动同步 — 0/6 Change archived；5 applied；1 进行中；Sprint `planning`
...
updated_at: 2026-07-26 16:05:07
---
...
| 类型 | 编号 | 标题 | 状态 | 估算 | 说明 |
|---|---|---|---|---:|---|
| REQ | REQ-0071-request-snapshot-logging | API 请求日志统一 Request Snapshot | in_sprint | 5.0 人天 | apply 22/22；待 archive `update-request-snapshot-logging` |
| REQ | REQ-0072-client-request-identity-standard | 前台后台与小程序统一客户端请求标识规范 | in_sprint | 5.0 人天 | apply 24/24；待 archive `standardize-client-request-identity` |
| REQ | REQ-0073-task-trace-parent-request-model | Task Trace 主请求与子请求关联模型 | in_sprint | 3.0 人天 | apply 25/25；待 archive `fix-task-trace-parent-request-model` |
| REQ | REQ-0074-task-trace-coverage-expansion | 任务型接口 Task Trace 覆盖扩展 | in_sprint | 5.0 人天 | apply 25/25；待 archive `update-task-trace-coverage-expansion` |
| REQ | REQ-0075-audit-log-task-trace-linking | 审计操作日志补齐任务链路关联字段 | in_sprint | 3.0 人天 | apply 19/19；待 archive `link-audit-logs-to-task-trace` |
| REQ | REQ-0076-observability-dashboard | 日志审计与链路观测仪表 | in_sprint | 5.0 人天 | in_progress 27/28；`add-observability-dashboard` |

BUG：无 已纳入正式范围，优先级高于新增体验能力；当前完成度与验收风险以 Scope 表状态、关联 Change 和 acceptance-report 为准。

Change：已回填 6 个范围项关联 Change；0 archived，5 applied，1 in_progress，0 proposed。所有已纳入范围项均已关联 Change；执行开发与归档时以 Scope 表逐项状态为准。
...
| 类型 | 路径 |
|---|---|
| REQ | `issues/requirements/review/REQ-0071-request-snapshot-logging/` |
| REQ | `issues/requirements/review/REQ-0072-client-request-identity-standard/` |
| REQ | `issues/requirements/review/REQ-0074-task-trace-coverage-expansion/` |
| REQ | `issues/requirements/review/REQ-0075-audit-log-task-trace-linking/` |
| Change | `openspec/changes/update-request-snapshot-logging/` |
| Change | `openspec/changes/standardize-client-request-identity/` |
| Change | `openspec/changes/update-task-trace-coverage-expansion/` |
| Change | `openspec/changes/link-audit-logs-to-task-trace/` |
