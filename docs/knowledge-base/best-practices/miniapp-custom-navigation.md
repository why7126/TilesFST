...
updated_at: 2026-08-04 09:20:00
...
- 使用防重复点击、loading、disabled、`navigating` 或等价导航锁时，验收必须覆盖真实状态流：首次点击成功、再次进入同一页面或另一页面、第二次点击仍可触发返回；仅检查源码中存在解锁逻辑不得作为唯一验收依据。
...
- [ ] 返回首页或返回兜底存在状态锁时，已覆盖同页重复进入、跨页重复进入、快速重复点击和失败兜底后的可恢复状态流。
