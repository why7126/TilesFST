...
| `follow_up` | 当前流程暂不可验证，但有后续承接动作 | 当前证据来源、不可验证原因、后续承接阶段、重试条件和剩余风险 |

`blocked` 不得视为通过。阻塞解除后必须重新记录对应维度结果。

环境相关媒体 evidence SHOULD 额外记录 `evidence_source`、`verification_boundary`、`evidence_ref` 和后续承接方式。开发阶段可用开发 API smoke、对象存储审计摘要、DevTools 截图或 DevTools Network 支撑开发验收；这些证据不得写作线上对象、线上域名、体验版或真机 Network 已通过。线上对象、线上接口、线上 no-fallback、缩略图回填或真实用户路径暂不可验证时，应记录当前证据来源、不可验证原因、后续承接方式或明确 N/A 理由。`production_only_pending` 仅作为历史兼容字段，不推荐新记录继续使用。
