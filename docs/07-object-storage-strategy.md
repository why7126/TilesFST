...
SKU 图片采用与原图同目录、文件名后缀差异化的三规格 Key 规则。原图
...
`images/default/tiles/{tile_id}/<uuid>.display.webp`。`thumbnails/` 前缀仅作为历史兼容读取或迁移来源，不作为新生成 SKU 多规格图的最终写入位置。
...
`object-key-audit` 为只读任务，不支持 apply。`backfill-brand-certificate-thumbnails`、`backfill-image-variants`、`formalize-pending-tile-images`、`migrate-certificate-image-keys` 与 `media-drift-reconcile` 默认 dry-run；生产写入必须追加 `--apply --confirm-backup`，确认 MySQL 与对象存储 bucket/prefix 已备份。历史缩略图重生成读取当前 `media.thumbnail_max_size_kb` effective 值，历史详情展示图重生成读取当前 `media.display_max_size_kb` effective 值；保存系统设置不会自动扫描、读取或覆盖历史 `.thumb` / `.display` 对象。`backfill-brand-certificate-thumbnails` 保留历史命令名，但实际覆盖 SKU、品牌 Logo 和品牌证书图片同目录 `.thumb.webp` 缩略图。`backfill-image-variants` 覆盖 SKU、品牌 Logo 和品牌证书图片同目录 `.thumb.webp` / `.display.webp` 多规格图 dry-run/apply、幂等统计和重试建议，并把存在但 MIME 不是 `image/webp` 的派生对象视为待重建。小程序证书卡片只消费同目录 `.thumb.webp` 或等价轻量缩略图 URL，缺缩略图或图片加载失败时展示占位，不得 fallback 到 `file_url`、原图或原文件 URL；小程序证书详情图片普通展示只消费同目录 `.display.webp` 或 `.thumb.webp` 受控 URL，点击预览才使用原图 URL；PDF/文档证书继续使用 `files/default/brand-certificates/` 或等价文件前缀，不生成图片 `thumbnail_url` 或 `display_url`。`media-drift-reconcile` 是生产推荐聚合入口，按顺序覆盖 SKU pending 主图正式化、证书图片 `files/` 到 `images/` key 迁移、SKU/品牌 Logo/证书图片同目录 `.thumb.webp` 缩略图回填和二次对象 key 审计；`bug-0116-media-drift` 仅作为历史兼容别名保留。维护输出只允许出现对象 Key hash、标准前缀、统计摘要、失败原因枚举和媒体验收摘要，不得输出 access key、secret key、数据库连接串、Authorization header、Cookie、真实 `.env` 内容、本机绝对路径或未脱敏 object key。
