...
Web 客户端 MUST 提供管理端登录页，路由为 `/admin/login`，视觉风格 MUST 高保真对齐 `issues/requirements/archive/REQ-0001-user-login/prototype/web/user-login.png` 暗色工业风设计。桌面端（1280px 视口）并排对比 PNG 时，布局、品牌字体、背景材质、控件形态、色彩与间距 MUST 被团队判定为一致或仅有可接受像素级偏差。页面 MUST 使用 Design Token semantic class，MUST NOT 依赖 JSX 裸 Hex。
...
- **AND** 结果 MUST 记录在 `openspec/changes/fix-login-pixel-fidelity/trace.md` 与 `iterations/archive/sprint-001/acceptance-report.md`
...
- **AND** MUST 指向 `issues/requirements/archive/REQ-0001-user-login/prototype/web/user-login.md` 作为详细 spec
