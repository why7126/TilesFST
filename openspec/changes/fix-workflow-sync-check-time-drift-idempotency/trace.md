...
| 类型 | ID | 文档 |
|---|---|---|
| BUG | BUG-0058-workflow-sync-check-time-drift-idempotency | `issues/bugs/review/BUG-0058-workflow-sync-check-time-drift-idempotency/` |
...
| 项目 | 状态 | 说明 |
|---|---|---|
| OpenSpec validate | pending | Change 创建后执行 |
| workflow-sync check | pending | 实现后连续执行并记录 no delta |
| Regression tests | pending | 实现后运行 `uv run pytest tests/test_workflow_sync_time_drift.py` |
