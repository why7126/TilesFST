...
- `issues/requirements/review/REQ-0037-auto-token-fact-source-for-workflow-commands/requirement.md`
...
- 建立统一 post-command AI usage hook，供所有 source-command 技能引用。
...
所有 source-command skill 只声明调用时机、输入参数和输出要求。具体 session 输入发现、`extract-ai-usage` 调用、归因、脱敏、写入和摘要输出集中在一个脚本、函数或共享规则中。
...
3. 更新 source-command 技能或共享规则，使 `/req-*`、`/bug-*`、`/opsx-*`、`/sprint-*` 在主流程和 Workflow Sync 成功后引用 hook。
