...
- [x] 3.1 在 `/admin/logs` 或等价管理端入口增加链路观测模式，保留日志列表与详情追踪入口。
- [x] 3.2 复用管理端 Design System、Admin Shell、筛选、指标卡、表格、分页、详情抽屉和 fixed toast 模式。
- [x] 3.3 实现统一摘要指标、失败原因分布、客户端分布、任务状态分布、慢任务排行、最慢 span 排行、慢请求排行和追踪 ID 查询区。
- [x] 3.4 实现筛选联动，确保摘要、分布、排行和明细入口共享同一筛选条件。
- [x] 3.5 实现排行项和追踪 ID 下钻，支持打开日志详情、Task Trace 时间线或相关记录集合。
- [x] 3.6 实现空数据、加载失败、无权限、追踪 ID 未命中状态，并确保聚合失败不破坏基础日志列表查询。
...
- [x] 5.2 前端测试覆盖页面渲染、筛选刷新、排行下钻、复制反馈、加载失败、无权限、空态和分页结构。
- [x] 5.3 验证 `admin-list` 横切 AC：指标卡 DOM、分页 DOM、fixed toast、无 `window.confirm`。
- [ ] 5.4 运行 1440x1024 与移动端管理端 smoke 或截图验收，覆盖摘要指标、筛选、排行、表格、详情入口和空态。
