...
### Requirement: 管理端链路观测仪表页面

系统 SHALL 在 Web 管理端提供链路观测仪表，使管理员可从聚合指标发现异常，并下钻到日志详情或 Task Trace 时间线。

#### Scenario: 管理员打开链路观测仪表
- **WHEN** 已认证 admin 打开 `/admin/logs` 或等价管理端日志审计入口
- **THEN** 页面 SHALL 提供链路观测模式
- **AND** 页面 SHALL 保留日志列表与详情追踪入口
- **AND** SYSTEM sidebar SHALL 仍按管理端日志审计或链路观测入口展示 active 态。

#### Scenario: 仪表展示摘要与筛选
- **WHEN** admin 查看链路观测仪表
- **THEN** 页面 SHALL 展示统一摘要指标、筛选区、失败原因分布、客户端分布、任务状态分布、慢任务排行、最慢 span 排行、慢请求排行和追踪 ID 查询区
- **AND** 筛选区 SHALL 支持时间范围、日志类型、客户端、任务类型、接口路径、状态或结果、`request_id` 和 `task_trace_id`。

#### Scenario: 观测数据与筛选口径一致
- **WHEN** admin 修改任一筛选条件
- **THEN** 页面 SHALL 使用同一筛选条件刷新摘要、分布、排行和明细入口
- **AND** 页面 SHALL NOT 展示与当前筛选条件不一致的缓存指标作为成功态。

#### Scenario: 排行项支持下钻
- **WHEN** admin 点击慢任务、失败任务、最慢 span、慢请求、错误接口或失败原因项
- **THEN** 页面 SHALL 打开对应日志详情、Task Trace 时间线或相关记录集合
- **AND** 页面 SHALL 保留当前筛选上下文。

#### Scenario: 复制追踪 ID 不造成布局位移
- **WHEN** admin 复制 `request_id` 或 `task_trace_id`
- **THEN** 页面 SHALL 优先写入系统剪贴板
- **AND** 成功、失败或兜底反馈 SHALL 使用 fixed toast 或等价固定层
- **AND** 反馈 SHALL NOT 推挤页面头部、筛选区、图表区、表格或分页区域。

#### Scenario: 页面异常状态
- **WHEN** 链路观测聚合接口返回空数据、加载失败、鉴权失败或追踪 ID 未命中
- **THEN** 页面 SHALL 分别展示零值 / 空集合、加载失败重试、无权限或无匹配追踪 ID 状态
- **AND** 聚合接口失败 SHALL NOT 破坏既有日志列表基础查询能力。

#### Scenario: 管理端 UI 横切验收
- **WHEN** 链路观测仪表实现完成
- **THEN** 摘要指标卡 SHALL 使用 `.metric-label`、`.metric-value` 和 `.metric-desc` 或等价 DS 结构
- **AND** 日志或任务明细分页 DOM SHALL 对齐用户管理基准：左侧 `.page-summary`，右侧 `.page-right` 页码与每页条数
- **AND** 页面实现 SHALL NOT 调用 `window.confirm`
- **AND** 1440x1024 与移动端管理端视口 SHALL 纳入页面级 smoke 或截图验收，覆盖摘要指标、筛选、排行、表格、详情入口和空态。

### Requirement: 链路观测契约同步

系统 SHALL 在实现链路观测仪表时同步维护 API、OpenAPI、Orval、文档、错误码、数据库说明和测试契约。
...
- **AND** 前端测试 SHALL 覆盖页面渲染、筛选刷新、排行下钻、复制反馈、加载失败、无权限、空态、分页结构、fixed toast 和移动端 smoke。
