...
- **AND** OPERATIONS MUST 包含「瓷砖规格」位于「瓷砖类目」与「Banner 管理」之间
...
#### Scenario: 瓷砖规格导航 active 态

- **WHEN** 用户位于 `/admin/tile-specs`
- **THEN** 「瓷砖规格」导航项 MUST 为 active 样式
- **AND** 「首页」与其他项 MUST 为非 active 样式
...
- **WHEN** 用户点击 Sidebar「瓷砖规格」
- **THEN** 系统 MUST 导航至 `/admin/tile-specs`
- **AND** MUST NOT 仅展示「功能建设中」占位 toast

#### Scenario: 其他未实现导航占位

- **WHEN** 用户点击除「首页」「瓷砖SKU」「瓷砖品牌」「瓷砖规格」「用户管理」（若 admin）外尚无实现的 Sidebar 项
