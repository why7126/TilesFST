...
- [x] 3.1 更新 `/req-*` 命令技能或共享规则，在 Workflow Sync 成功后引用统一 hook。
- [x] 3.2 更新 `/bug-*` 命令技能或共享规则，在 Workflow Sync 成功后引用统一 hook。
- [x] 3.3 更新 `/opsx-*` 命令技能或共享规则，在 Workflow Sync 成功后引用统一 hook。
- [x] 3.4 更新 `/sprint-*` 命令技能或共享规则，保持与 REQ-0035 的 Sprint snapshot gate 一致。
