...
当前 Sprint 归档 readiness gate 主要校验 Change 目录是否存在以及 `tasks.md` checkbox 是否全部完成。对于已归档到 `openspec/archive/YYYY-MM-DD-<change-id>/` 的 Change，缺少 `trace.md` 并不会被自动阻断；如果 `proposal.md`、`design.md`、`tasks.md` 也没有统一的归档验证摘要，后续审计无法稳定追溯验证结论、测试命令、Issue/Sprint 状态和归档证据。
