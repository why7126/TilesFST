...
#### Scenario: 历史缩略图审计与重生成
- **GIVEN** 存量公开 SKU 已存在原图和同目录 `.thumb` 对象
- **WHEN** 运维执行历史缩略图审计 dry-run
- **THEN** 输出 SHALL 包含原图存在、缩略图存在、疑似同 size、疑似同 bytes、需要重生成、跳过、失败原因等摘要
- **AND** dry-run SHALL NOT 写数据库或对象存储
- **WHEN** 运维执行重生成 apply
- **THEN** 系统 SHALL 只重生成需要处理的 `.thumb` 对象
- **AND** 重复执行 SHALL 保持幂等，不破坏已合格缩略图
- **AND** 输出 SHALL NOT 泄露密钥、Authorization header、Cookie、`.env` 内容或本机绝对路径。
