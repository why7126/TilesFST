...
REQ-0075 来自已评审需求 `issues/requirements/archive/REQ-0075-audit-log-task-trace-linking/`。平台已经有 `product-usage-logging` 能力、日志审计页、Task Trace 模型，以及已归档的 `add-task-trace-audit-log-view` 变更；当前缺口不在“是否能展示 Task Trace”，而在 `audit_logs` 写入链路没有稳定接收和持久化 `task_trace_id` / `task_type`。
