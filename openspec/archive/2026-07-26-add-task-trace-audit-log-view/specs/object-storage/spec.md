...
超限、MIME 不符或对象存储不可用 MUST 由后端返回统一结构错误响应和明确错误码，MUST NOT 依赖 Nginx 413 作为业务校验手段。Docker / Nginx / 生产代理请求体大小和超时配置 MUST 大于等于后端最大 effective 上传限制。图片、视频和文件上传作为首批 Task Trace 场景时，上传链路 MUST 记录或关联同一个 `task_trace_id`，并记录前端、后端、对象存储、数据库和后处理关键节点。
...
- **AND** 上传任务 SHALL 可通过 `task_trace_id` 追踪。
...
- **WHEN** 管理端上传图片、视频或文件
- **THEN** 系统 SHALL 记录或关联 `frontend_upload_start`、`frontend_upload_body_done`、`api_receive`、`validate_file`、`storage_put_object`、`db_create_media`、`api_response` 和前端完成或失败节点
- **AND** 无后处理时 `post_process` MAY 标记为 N/A。

#### Scenario: 99% 上传停留可拆解

- **WHEN** 前端上传进度达到 99% 但接口尚未返回
- **THEN** 系统 SHALL 能通过 `task_trace_id` 统计从 `frontend_upload_body_done` 到 `api_response` 的耗时
- **AND** SHALL 能定位对象存储写入、数据库落库、后处理或响应返回中的最慢节点。
...
- **AND** 验收 SHALL 经 `http://localhost:3000` 覆盖，不得只验证后端 `:8000`。
...
- **AND** MUST NOT 因默认 60 秒反代超时返回 504
