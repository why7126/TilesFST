...
#### Scenario: 品牌编辑弹窗不显示冗余 Logo 成功态文案
- **GIVEN** 品牌记录存在有效 Logo
- **WHEN** `admin` 或 `employee` 打开品牌编辑弹窗
- **THEN** Logo 上传/预览区域 MUST NOT 显示 `已上传Logo`、`品牌Logo` 或等价冗余成功态文案
- **AND** 当前 Logo 图片预览 MUST 仍正常展示
- **AND** Logo 图片顶部 MUST 与格式提示文字顶部视觉对齐
- **AND** 替换、删除或重新上传 Logo 的入口 MUST 保持可用
- **AND** 上传中、上传失败、格式不支持等必要状态反馈 MUST 继续可见。
...
- **AND** 稳定预览状态 MUST NOT 展示 `已上传Logo` 或等价冗余成功态文案。
