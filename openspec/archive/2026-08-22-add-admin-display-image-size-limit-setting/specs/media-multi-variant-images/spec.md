...
- **AND** `.display` 生成 MUST 读取 `media.display_max_size_kb` 或等价 effective 配置
- **AND** `.thumbnail` 生成 MUST 继续读取 `media.thumbnail_max_size_kb` 或等价缩略图配置
...
#### Scenario: display 体积目标无法达标时保留业务可用性

- **WHEN** `.display` 派生生成因 PNG、透明图、复杂纹理或压缩极限无法达到 configured target
- **THEN** 系统 MUST NOT 阻断原图上传、业务保存或维护任务整体执行
- **AND** 系统 MUST 写入最优可用 display、跳过 display 或返回安全降级结果
- **AND** warning、生成状态、失败原因或维护任务摘要 MUST 可复核。
