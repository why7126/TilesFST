...
验收方式：2026-06-20 代码/CSS Port 结构对照 v2 HTML + vitest/pytest/build + Docker 冒烟。

| # | 检查项 | Pass |
|---|--------|------|
| 1 | Admin Shell：264px Sidebar + 右侧滚动 | pass |
| 2 | Logo TILESFST；「用户管理」菜单 active | pass |
| 3 | 页面标题「用户管理」+ 添加用户品牌金按钮 | pass |
| 4 | 筛选区 5 列网格（无搜索按钮） | pass |
| 5 | placeholder「搜索用户名/昵称」 | pass |
| 6 | 重置按钮存在且样式一致 | pass |
| 7 | 4 指标卡布局与 v2 一致 | pass |
| 8 | 指标卡与表格之间无「用户列表」标题行 | pass |
| 9 | 表格内无 toolbar 提示行 | pass |
| 10 | 表头六列：用户、角色、状态、最后登录、创建时间、操作 | pass |
| 11 | 用户列：头像 + 用户名/昵称两行纵向 | pass |
| 12 | 空昵称显示「未设置昵称」；无邮箱副标题 | pass |
| 13 | 分页左「共 x 个用户」 | pass |
| 14 | 分页右：页码 +「每页显示」条数 | pass |
| 15 | 无「1-10 / N」「当前显示」文案 | pass |
| 16 | 角色/状态 badge 颜色与 v2 一致 | pass |
| 17 | 行操作链接样式（编辑/重置/冻结/删除）无回归 | pass |
...
cd src/web && npx vitest run src/pages/admin/UserManagementPage.test.tsx
...
- PNG golden reference 待人工导出（5.3 可选）。
...
| 2026-06-20 | `/sprint-apply` | v2 列表 UI + keyword 收窄 + 测试 |
