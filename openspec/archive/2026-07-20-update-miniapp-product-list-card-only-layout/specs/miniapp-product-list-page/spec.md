...
### Requirement: 筛选与排序
**Reason**: REQ-0056 将商品列表页定位为轻量商品浏览页，页面内不再展示搜索、筛选或排序控件，避免控件占用首屏并与首页热销推荐式双列浏览心智冲突。

**Migration**: 商品列表页继续保留入口上下文和默认查询；搜索、筛选和搜索结果能力保留在微信小程序搜索页或入口前置页面。后端商品列表接口可继续保留兼容参数，供搜索页或未来能力使用。
