...
Sprint 归档会将四件套从 `iterations/change/<sprint-id>/` 迁移到 `iterations/archive/<sprint-id>/`，关联 Change 也会进入 `openspec/archive/<date>-<change-id>/`。当前 `/sprint-archive` 与 `/sprint-exps` 缺少针对旧路径引用的显式残留检查，容易在归档后留下指向 `change/` 或 active Change 目录的旧链接，影响后续追溯与复盘证据可信度。
