...
| 场景 | 做法 |
|------|------|
| 绿场首次建设 | `/req-capture` … `/req-review` → `/sprint-propose` → `/req-opsx` → `add-design-system` 或 `build-design-system` |
...
```text
/req-capture → … → /req-review → /sprint-propose sprint-xxx --req REQ-xxxx → /req-opsx REQ-xxxx
```
