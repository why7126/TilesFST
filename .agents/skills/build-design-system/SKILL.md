...
| 场景 | 做法 |
|------|------|
| 绿场首次建设 | `/req-capture` … `/req-review --approve` → `/sprint-propose` → `/req-opsx` → `add-design-system` 或 `build-design-system` |
| 已归档（本项目） | 直接按本命令补代码；**规范变更**须新 REQ + `/sprint-propose` + `/req-opsx` |
...
```text
/req-capture → … → /req-review --approve → /sprint-propose sprint-xxx --req REQ-xxxx → /req-opsx REQ-xxxx
```
