...
Use this skill when the user asks to run the workflow command `opsx-explore`.
...
```bash
python scripts/extract-ai-usage.py --post-command-hook --workflow-event opsx.explore --change <change-id> --dry-run --json
```

- If no Change ID is relevant, omit `--change`.
