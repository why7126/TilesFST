...
若 REQ 涉及 API、DB、日志审计、行为埋点、Task Trace、Web 请求封装、小程序请求封装或 App 请求封装，MUST 读取 `docs/standards/product-data-collection-observability.md`，并在 `trace.md` 写入：

```yaml
product_data_collection_observability:
  status: applicable | not_applicable
  affected_layers: []
  reason: <适用或 N/A 的具体原因>
  validation: <后续验证或验收摘要>
```

`acceptance.md` MUST 包含可验收项，覆盖适用层级、N/A 原因、API/DB/端请求封装、脱敏、保留周期、测试和 Orval 影响。N/A 原因不得只写“无”或“不涉及”。
...
2. 通过后先 `/sprint-propose sprint-xxx --req REQ-xxxx`，再 `/req-opsx REQ-xxxx`（design.md MUST 引用 knowledge_base_refs）
